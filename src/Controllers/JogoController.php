...
function processarAcoesDoJogo($cenas)
{
    ...
    if (isset($_POST["salvarRanking"])) {
        salvarPontuacaoRanking(isset($_POST["nomeRanking"]) ? $_POST["nomeRanking"] : "", obterPontuacao());
    }

    if (isset($_POST["novoJogo"])) {
        reiniciarJogo();
        return array("mensagensBatalha" => array(), "acabouDeVencer" => false, "acabouDeFugir" => false);
    }

    if (!jogoFoiIniciado()) {
        return array("mensagensBatalha" => $mensagensBatalha, "acabouDeVencer" => false, "acabouDeFugir" => false);
    }
    ...
    if (isset($_POST["jogarDado"]) && $desafioPendente && !jogoFoiPerdido()) {
        $resultadoDados = jogarRodadaDados($idCenaAtual, $cenas);
    ...
    if ($temBatalha && !$desafioPendente && !batalhaEncerrada($idCenaAtual)) {
        ...
        if (isset($_POST["reiniciarBatalha"])) {
        ...
        } elseif ($idCenaAtual == 11 && isset($_POST["escolhaJokenpo"])) {
            $resultadoAtaque = jogarJokenpo($_POST["escolhaJokenpo"]);
            $mensagensBatalha = $resultadoAtaque["mensagens"];
            if ($resultadoAtaque["vitoria"]) { $acabouDeVencer = true; }
            ...
            $mensagensBatalha = $resultadoAtaque["mensagens"];
            if ($resultadoAtaque["fuga"]) { $acabouDeFugir = true; }
        } else {
            if (!batalhaPerdida($idCenaAtual)) {
                $mensagensBatalha[] = "Um inimigo apareceu: " . $_SESSION["nomeInimigo"] . ".";
                $mensagensBatalha[] = "Escolha sua ação. Depois disso, o inimigo escolherá aleatoriamente ATACAR ou DEFENDER.";
                $mensagensBatalha[] = "FUGIR usa 1d20, funciona somente com 16 ou mais e pode ser tentado uma única vez por batalha.";
            }
        }
    }
